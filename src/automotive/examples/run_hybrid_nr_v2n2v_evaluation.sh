...
RMR_GUARD_IMPORTANT=${RMR_GUARD_IMPORTANT:-false}
RMR_GUARD_HARD_DISTANCE=${RMR_GUARD_HARD_DISTANCE:-30}
RMR_GUARD_HARD_TTC=${RMR_GUARD_HARD_TTC:-3}
...
MEC_IDEAL_RMR_DELETE_VERY_HIGH=${MEC_IDEAL_RMR_DELETE_VERY_HIGH:-80}
MEC_IDEAL_RMR_DELETE_EXTREME=${MEC_IDEAL_RMR_DELETE_EXTREME:-160}
...
TRAFFIC_FLOW_RSU_PREDICTOR=${TRAFFIC_FLOW_RSU_PREDICTOR:-false}
...
LOW_PRIORITY_CPM_RECOGNITION_TTL=${LOW_PRIORITY_CPM_RECOGNITION_TTL:-0.2}
...
for method in $METHODS; do
  ...
  mkdir -p "$RUN_DIR"
  {
    ...
    echo "MEC_IDEAL_RMR_DELETE_EXTREME=$MEC_IDEAL_RMR_DELETE_EXTREME"
    echo "MEC_IDEAL_LINK=$MEC_IDEAL_LINK"
    echo "MEC_IDEAL_DELAY_MODEL=$MEC_IDEAL_DELAY_MODEL"
    echo "MEC_IDEAL_AOI_FILTER=$MEC_IDEAL_AOI_FILTER"
    ...
    echo "SENSOR_RANGE=$SENSOR_RANGE"
    echo "PRIORITY_DISTANCE=$PRIORITY_DISTANCE"
    echo "PRIORITY_CLOSING_SPEED=$PRIORITY_CLOSING_SPEED"
    echo "PRIORITY_TTC=$PRIORITY_TTC"
    echo "NR_BG=$NR_BG"
    echo "NR_BG_PER_VEHICLE=$NR_BG_PER_VEHICLE"
    echo "NR_BG_SIZE=$NR_BG_SIZE"
    echo "NR_BG_INTERVAL_MS=$NR_BG_INTERVAL_MS"
    echo "MEC_BG=$MEC_BG"
    echo "MEC_BG_PER_VEHICLE=$MEC_BG_PER_VEHICLE"
    echo "MEC_BG_SIZE=$MEC_BG_SIZE"
    echo "MEC_BG_INTERVAL_MS=$MEC_BG_INTERVAL_MS"
  ...
  /usr/bin/time -v -o "$RUN_DIR/resource_usage.txt" ./ns3 run "v2v-hybrid-nr-v2n2v \
    ...
    --rmr-guard-important=$RMR_GUARD_IMPORTANT \
    --rmr-guard-hard-distance=$RMR_GUARD_HARD_DISTANCE \
    ...
    --mec-ideal-rmr-longtail-extreme-cbr=$MEC_IDEAL_RMR_LONGTAIL_EXTREME_CBR \
    --mec-ideal-rmr-delete-very-high=$MEC_IDEAL_RMR_DELETE_VERY_HIGH \
    --mec-ideal-rmr-delete-extreme=$MEC_IDEAL_RMR_DELETE_EXTREME \
    ...
    --traffic-flow-rsu-predictor=$TRAFFIC_FLOW_RSU_PREDICTOR \
