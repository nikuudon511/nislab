...
/*
 * Hybrid V2V route-control example.
 *
 * This example keeps the radio and ETSI stack behavior inside VaN3Twin/ns-3
 * modules. 802.11p/ITS-G5 is configured as in v2v-congestion-80211p, NR-V2X
 * sidelink is configured as in v2v-coexistence-80211p-nrv2x, and the only
 * logic in this file is selecting which already-configured Basic Service
 * container is currently allowed to transmit.
 */
...
NS_LOG_COMPONENT_DEFINE ("V2VHybridCbr80211pNrv2x");
  ...
  for (const auto& entry : g_vehicleRuntime)
    {
      ...
      std::set<int> ldmIds;
      auto collectLdmIds = [&ldmIds] (Ptr<BSContainer> container) {
        if (container != nullptr && container->getLDM () != nullptr)
          {
            container->getLDM ()->getAllIDs (ldmIds);
          }
      };
      collectLdmIds (entry.second.dsrcContainer);
      collectLdmIds (entry.second.nrContainer);
          collectLdmIds (entry.second.mecContainer);

      std::set<uint64_t> ldmRecognizedStationIds;
      uint32_t recognizedObjects = 0;
      uint32_t highPriorityRecognizedObjects = 0;
      uint32_t lowPriorityRecognizedObjects = 0;
      for (int ldmId : ldmIds)
        {
          if (ldmId <= 0)
            {
              continue;
            }
          const uint64_t stationId = static_cast<uint64_t> (ldmId);
          if (expectedStationIds.count (stationId) > 0)
            {
              ldmRecognizedStationIds.insert (stationId);
              ++recognizedObjects;
            }
      ...
      std::set<uint64_t> sensorAndCpmRecognizedStationIds = sensorRecognizedStationIds;
      sensorAndCpmRecognizedStationIds.insert (ldmRecognizedStationIds.begin (),
                                               ldmRecognizedStationIds.end ());
      uint32_t sensorHighPriorityRecognizedObjects = 0;
      uint32_t sensorLowPriorityRecognizedObjects = 0;
      for (uint64_t stationId : sensorAndCpmRecognizedStationIds)
        {
          if (highPriorityExpectedStationIds.count (stationId) > 0)
            {
              ++sensorHighPriorityRecognizedObjects;
            }
          else if (expectedStationIds.count (stationId) > 0)
            {
              ++sensorLowPriorityRecognizedObjects;
            }
        }

      g_thesisStats.sensorRecognitionRatioSum +=
          Clamp01 (static_cast<double> (sensorAndCpmRecognizedStationIds.size ()) /
                   static_cast<double> (expectedStationIds.size ()));
      ++g_thesisStats.sensorRecognitionVehicleSamples;
      if (!highPriorityExpectedStationIds.empty ())
        {
          g_thesisStats.sensorHighPriorityRecognitionRatioSum +=
              Clamp01 (static_cast<double> (sensorHighPriorityRecognizedObjects) /
                       static_cast<double> (highPriorityExpectedStationIds.size ()));
          ++g_thesisStats.sensorHighPriorityRecognitionVehicleSamples;
        }
      const uint32_t sensorLowPriorityExpectedObjects =
          static_cast<uint32_t> (expectedStationIds.size () -
                                 highPriorityExpectedStationIds.size ());
      if (sensorLowPriorityExpectedObjects > 0)
        {
          g_thesisStats.sensorLowPriorityRecognitionRatioSum +=
              Clamp01 (static_cast<double> (sensorLowPriorityRecognizedObjects) /
                       static_cast<double> (sensorLowPriorityExpectedObjects));
      ...
      if (!cooperativeExpectedStationIds.empty ())
        {
          ...
          for (uint64_t stationId : ldmRecognizedStationIds)
            {
...
OpenObservationLog (const std::string& path)
{
  ...
  g_observationLog
      << "time_s,active_vehicles,dsrc_route_count,nr_route_count,mec_route_count,dual_route_count,dsrc_cbr_avg,dsrc_cbr_max,"
      << "dsrc_predicted_cbr_avg,dsrc_predicted_cbr_max,dsrc_cbr_rate_avg,"
      << "dsrc_cpm_tx_rate_avg,traffic_q_in,traffic_q_out,traffic_flow_cbr,traffic_delta_cbr,"
      ...
      << "dsrc_prr_avg,dsrc_packet_loss_avg,dsrc_latency_ms,dsrc_tx,dsrc_rx,"
      << "nr_prr_avg,nr_packet_loss_avg,nr_latency_ms,nr_tx,nr_rx,"
      << "mec_prr_avg,mec_packet_loss_avg,mec_latency_ms,mec_tx,mec_rx,"
      << "dsrc_cpm_objects,dsrc_rmr_candidates,dsrc_rmr_deleted_last,"
      << "dsrc_cpm_objects_total,dsrc_rmr_candidates_total,dsrc_rmr_deleted_total,"
      << "dsrc_cpm_size_bytes,dsrc_cpm_size_bytes_total,nr_cpm_objects,nr_rmr_candidates,"
               ...
               bool force = false)
{
  ...
  const double desync = (std::stoul (vehicleId.substr (3)) % 100) / 100.0;
                 ...
                 bool force = false)
{
  ...
  if (mecActive && runtime.mecContainer == nullptr)
    {
  ...
  for (auto& entry : g_vehicleRuntime)
    {
      ...
      SetRmrForwardingModes (runtime,
                             CPBasicService::RMR_FORWARDING_PRIMARY,
                             g_hybridRouteConfig.mecRecoveryPolicy == MecRecoveryPolicy::DuplicateAlways
                                 ? CPBasicService::RMR_FORWARDING_DUPLICATE
                                 : CPBasicService::RMR_FORWARDING_OFFLOAD);
      SetRouteTxState (vehicleId,
                       true,
...
main (int argc, char* argv[])
{
  ...
  bool enableDsrcInterference = true;
  ...
  std::string cbrLogPath = "hybrid-cbr-80211p-log.csv";
  ...
  cmd.AddValue ("dsrc-interference", "Enable real 802.11p PacketSocket background traffic", enableDsrcInterference);
  cmd.AddValue ("dsrc-interference-source", "Deprecated single-vehicle background source option", dsrcInterferenceSourceVehicle);
  cmd.AddValue ("dsrc-interference-nodes", "Number of SUMO vehicle nodes sending 802.11p background traffic", dsrcInterferenceNodeCount);
  ...
  const bool enableNrSidelinkV2v = enableRouteControl && !enableMecV2n2v;
  if (mecForwardRangeMeters == 0.0)
    {
  ...
  std::cout << "Vehicles=" << numberOfNodes << ", initial route=DSRC/802.11p"
            << ", optional fallback route="
            << (enableMecV2n2v ? "MEC V2N2V over LTE Uu/EPC"
                                : (enableNrSidelinkV2v ? "NR-V2X sidelink V2V"
                                                       : "disabled"))
            << std::endl;
  std::cout << "Method: " << method << std::endl;
  std::cout << "802.11p PHY mode: " << phyMode << ", DCC bitrate="
            << dsrcDccBitRateMbps << " Mbps" << std::endl;
  std::cout << "DSRC interference: " << (enableDsrcInterference ? "enabled" : "disabled");
  if (enableDsrcInterference)
    {
  ...
  std::cout << "DSRC exists in VaN3Twin as IEEE 802.11p/ITS-G5 via wave + GeoNet/BTP."
            << std::endl;
  ...
  Ptr<LteHelper> mecLteHelper;
  Ptr<PointToPointEpcHelper> mecEpcHelper;
  ...
  if (enableMecV2n2v)
    {
      Config::SetDefault ("ns3::LteEnbRrc::SrsPeriodicity",
                          UintegerValue (mecSrsPeriodicity));
      mecLteHelper = CreateObject<LteHelper> ();
      mecEpcHelper = CreateObject<PointToPointEpcHelper> ();
      mecLteHelper->SetEpcHelper (mecEpcHelper);
      mecLteHelper->SetHandoverAlgorithmType ("ns3::A3RsrpHandoverAlgorithm");
      mecLteHelper->SetHandoverAlgorithmAttribute ("Hysteresis", DoubleValue (3.0));
      mecLteHelper->SetHandoverAlgorithmAttribute ("TimeToTrigger",
      ...
      mecEnbDevices = mecLteHelper->InstallEnbDevice (mecEnbNodes);
      mecLteHelper->AddX2Interface (mecEnbNodes);
    }
  ...
  if (enableMecV2n2v)
    {
      mecUeDevices = mecLteHelper->InstallUeDevice (vehicleNodes);
    }
  ...
  if (enableDsrcInterference)
    {
      ...
      for (uint32_t i = 0; i < interferenceSocketCount; ++i)
        {
          Ptr<Node> sourceNode = vehicleNodes.Get (i);
          Ptr<NetDevice> sourceDevice = dsrcDevices.Get (i);
          Ptr<NetDevice> receiverDevice =
              dsrcDevices.Get ((i + 1) % interferenceSocketCount);
          Ptr<Socket> socket =
              Socket::CreateSocket (sourceNode, TypeId::LookupByName ("ns3::PacketSocketFactory"));

          PacketSocketAddress local;
          local.SetSingleDevice (sourceDevice->GetIfIndex ());
          local.SetPhysicalAddress (sourceDevice->GetAddress ());
          local.SetProtocol (0x88B5);
          if (socket->Bind (local) == -1)
            {
              NS_FATAL_ERROR ("Failed to bind DSRC background PacketSocket");
            }

          PacketSocketAddress remote;
          remote.SetSingleDevice (sourceDevice->GetIfIndex ());
          remote.SetPhysicalAddress (receiverDevice->GetAddress ());
          remote.SetProtocol (0x88B5);
          socket->Connect (remote);
          socket->SetPriority (dsrcInterferenceUserPriority);
          dsrcInterferenceSockets.push_back (socket);
        }
    }
  ...
  if (enableMecV2n2v)
    {
      ...
      mecLteHelper->AttachToClosestEnb (mecUeDevices, mecEnbDevices);
                                     ...
                                     TraciClient::StationTypeTraCI_t stationType) -> Ptr<Node>
  {
    ...
    const unsigned long stationId = std::stoul (vehicleId.substr (3));
    ...
    ActivateRoute (vehicleId, ActiveRoute::DsrcV2v, -1.0, "initial_route", true);
    if (enableDsrcInterference &&
  ...
  std::cout << "MEC V2N2V average packet loss: "
            ...
            << std::endl;
  std::cout << "MEC V2N2V TX/RX: " << mecMetrics->getNumberTx_overall () << "/"
            << mecMetrics->getNumberRx_overall () << std::endl;
  std::cout << "MEC V2N2V average latency (ms): "
            << mecMetrics->getAverageLatency_overall () << std::endl;
  ...
  if (!summaryCsvPath.empty ())
    {
      ...
      summaryCsv
          ...
          << "dsrc_route_loss,nr_route_loss,mec_route_loss,dsrc_cbr,dsrc_tx,dsrc_rx,"
          << "mec_tx,mec_rx,mec_latency_ms,interference_tx,interference_drops,"
      ...
      summaryCsv << method << "," << FormatThesisMetric (recognitionRate) << ","
                 ...
                 << "," << dsrcMetrics->getAverageCBROverall () << ","
                 << dsrcMetrics->getNumberTx_overall () << ","
                 << dsrcMetrics->getNumberRx_overall () << ","
                 << mecMetrics->getNumberTx_overall () << ","
                 << mecMetrics->getNumberRx_overall () << ","
                 << mecMetrics->getAverageLatency_overall () << "," << g_interferenceTx << ","
                 << g_interferenceDrops << "," << g_interferenceBytes << std::endl;
    }
